...
| `src/dashboard.html` | The real candidate dashboard (Sprint 5) — profile summary, badges (read-only), a per-section "at a glance" list with edit links back into the wizard, posts (compose/list/delete), incoming shortlists + consent toggle (Sprint 9), account closure. This is the wireframe's **Profile** tab (Sprint 19) — the tab-bar shell now sits at the bottom of every signed-in page instead of a text nav link. Its own inline shortlist section is unchanged/still functional, not yet consolidated into `invites.html`/`pipelines.html`. **2026-09-13** — header rebuilt: real name (`GET /auth/me`, not the `headline` field) → current position (from `employment_history`, `is_current`) → location (town only, the postcode-district bug is fixed) → a two-tier identity check, icon+colour not text (migration 0042, superseding the same-day 0041 single-tier version — see PROGRESS.md): purple "Fully Verified" (`id_verified` + `dbs_certificate_verified` badges + right_to_work stated) or teal "Identity Verified" (`id_verified` only) → right-to-work + an honest "ID on file: Not yet collected" line (no passport/ID upload exists anywhere in this app, by deliberate choice — see PROGRESS.md). Both new badges are manual-only (Supabase-dashboard insert into `candidate_badges`, no admin UI) |
