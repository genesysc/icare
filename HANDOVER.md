...
| `src/landing.html` | **Rebuilt 2026-09-14.** The public launched-state landing page at `/` — single self-contained file, inline CSS, three bundled photos from `src/assets/`, and (deliberately) minimal JavaScript: the magic-link recovery below, plus (2026-09-16) a small mobile-nav toggle. Positioning comes from the founder's market-research handover: *"iCare is your passport for your next job in health and care"* — verify once, carry that trust to every employer. Sections: hero → the problem (three cited stats) → how it works (three steps) → badge grades (dark band) → precedent (NHS staff banks prove the model, scoped to NHS temp/bank only) → who it's for → what we will never do → final CTA → footer. CTAs are real product entry points (`/sign-up`, `/sign-in`); the waitlist capture form that used to live here is **gone** (the `POST /waitlist` API and the employer page's own form are untouched). **2026-09-18: all employer nav/promotion removed from this page** — founder decided candidates shouldn't be shown employer-facing content on the candidate consumer site at all, temporarily, while iRecruit's B2B side stays sales-led/manually onboarded rather than self-serve (matches the standing multi-vertical strategy of iRecruit eventually living on its own domain, §14). Removed: the top-nav and mobile-nav "For employers" link, and the mid-page "Hiring for a health or care team? → See iCare for employers" strip section (and its now-dead `.employers` CSS). The footer's "Employers" column was **not** removed outright (temporary, not permanent, per the founder) — relabeled "Hiring?" with a single link to `/employers`, which is itself already waitlist-only marketing copy (no self-serve sign-up funnel exists there — see that file's own header comment), replacing the old "Employer log in" link that went straight to the real, working `/employer/sign-in`. **`/employer/sign-in`/`/employer/sign-up`/`/employer/home` themselves are untouched and still fully functional** — this was a promotion-only change, not an access change, so an already-verified employer or anyone sent a direct link can still sign in. `privacy.html`/`terms.html` still carry a footer "For employers" link — deliberately left alone, out of scope for this request (landing page only). Still recovers a magic-link sign-in that lands here by mistake (2026-09-02 fix, see §14/PROGRESS.md) — checks for `#access_token=` in the URL hash on load, before anything else runs, and forwards to `/verify`. **Keep that script first in `<head>` through any future rewrite.** **2026-09-16**: the mobile nav (<680px) used to just delete "For employers"/"Insights"/"Log in" from the page via `display:none` with nothing to replace them, so they were only reachable by scrolling to the footer — replaced with a real hamburger-to-X toggle (`[data-nav-toggle]`/`[data-mobile-nav]`) that reveals them in a dropdown; "Sign up" stays inline as the one always-visible primary action. The footer was also rebuilt from a single centered line of 5 links into a real one — brand blurb + Candidates/Employers/Legal columns + a copyright bar. Verified with Playwright screenshots at 375px/1280px before shipping (PROGRESS.md's 2026-09-16 entry). |
...
| `src/member.html` | **New 2026-09-17** — "view another member's profile," `/member?id=<candidate_id>`. Closes the gap the Rounds/Network/Messages/Profile spec and `handover_3.md` both explicitly flagged as undesigned. Read-only, LinkedIn-shaped like `dashboard.html`'s self-view (About/Badges/Experience/Qualifications & Registrations/Skills/Activity) but with no Edit links, no Employer interest/Visibility/Account cards, and — deliberately — no Right to work / ID line and no DBS anywhere (DBS stays employer-only per non-negotiable #3; RTW and raw registration numbers are withheld from peers too, a reasonable default this session made rather than a documented decision — see §12). Connect/Message actions reuse the exact same states as `network.html`. Linked from `network.html` (every person-item name) and `rounds.html` (post author name + avatar) so it's actually reachable, not just a dead-end route. Backed by `GET /candidates/:id/profile` (`candidates.ts`) reading `candidate_peer_profile`/`candidate_peer_registrations` (migration `0047`) plus the existing `candidate_peer_feed`/`candidate_badges`. **Real bug found and fixed 2026-09-18 (migration `0049`)**: `0047`'s Experience/Qualifications/Skills peer-read policies each gated on an inline `exists (select 1 from candidates c where ... c.is_published)` — but that subquery runs under the *viewer's own* RLS on `candidates`, which has never let one candidate read another's row directly (only self/verified-employer, `0002`), so it silently saw zero rows for every peer and those three sections rendered empty for everyone (About/Badges still worked — different code paths, a view and a public-read table respectively). Same bug class migration `0029` already hit once before; fixed the same way, using the existing `candidate_is_published()` SECURITY DEFINER helper instead of a second inline subquery. Verified by simulating RLS as a real candidate role in SQL before and after (0/0/0 rows → 1/1/15, matching the actual data) — not just re-reading the migration. **2026-09-18**: Activity card now always renders with a "No posts yet." empty state instead of hiding the whole card when a candidate has none (founder feedback — a viewer should be told there are no posts, not left wondering if the section failed to load) — matches `dashboard.html`'s own `data-posts-empty` convention exactly. |
...
- Notifications (2026-09-15): a real notification center, candidate-side
  ...
  PROGRESS.md's 2026-09-15 entry. **Gap found and backfilled 2026-09-16**:
  the four triggers only fire on new inserts, so anything already
  pending/sent before migration 0043 (2026-09-15) had no notification and
  never could get one retroactively — migration 0046 backfilled the 3
  affected rows repo-wide (all pending connection requests; nothing else
  was missing). A future one-off insert done outside these triggers (e.g.
  another raw-SQL seed) would reintroduce the same class of gap — there's
  no trigger-independent safety net here by design, same tradeoff as
  everything else in this schema that leans on SECURITY DEFINER triggers
  as "the only way in."
...
| **Viewing another member's profile** | Was the one genuine gap the spec itself flagged as undesigned — **closed 2026-09-17**, `src/member.html`, migration `0047`. Scoping (no RTW/reg numbers peer-side) founder-confirmed 2026-09-18, see §12. Shipped with a real RLS bug (Experience/Qualifications/Skills silently empty for every viewer) — **fixed 2026-09-18, migration `0049`**, see the `member.html` file-map entry above. |
...
## 19. Candidate Home ("Rounds") news feed module — 2026-09-18/19
